...
/**
 *clase para modelar un dado de figuras, los dados que ensenan figuras
 * @author ACR
 */
public final class DadoFigura extends Dado {

    private String figura;
    private final String[] figurasDisponible;
    //constructor
    public DadoFigura(int NUMERODECARAS, int x, int y, int width, int height, String[] colores, String[] figuras) {
        super(NUMERODECARAS, x, y, width, height, colores);
        figurasDisponible = figuras;//para saber que figuras se pueden usar
    ...
    public void setImagenes() {
        try {
            imagenDado = ImageIO.read(new File("Imagenes/" + color + "/" + figura + ".png"));
            if (lanzable == true) {
    ...
    public void lanzar() {
        if (lanzable == true)//si se puede lanzar el dado se lanzara, de lo opuesto no.
        {
            ...
            color = coloresDisponible[rmd.nextInt(7)];
            figura = figurasDisponible[valorCaraMostrada - 1];
            setImagenes();
        }
    }
